...
def edit_entry(entries):
    ...
    entry = next((e for e in entries if e["Seq"] == seq), None)
    if not entry:
        print("❌ No entry found with that Seq #")
        return entries

    print("\n=== Editing Entry ===")
    print(f"{entry['Seq']:<5}{entry['Date']:<12}{entry['Person']:<20}{entry['Type']:<15}{entry['Amount']:<10}{entry['RunningBalance']:<12}{entry['Description']}")

    def prompt(field, old):
        new = input(f"{field} [{old}]: ").strip()
        return new if new else old

    new_date = prompt("Date (dd-Mon-yy)", entry["Date"])
    new_person = prompt("Person/Shop", entry["Person"])
    new_type = prompt("Type (Contribution/Expense)", entry["Type"])
    new_amount_str = prompt("Amount", entry["Amount"])
    new_desc = prompt("Description", entry["Description"])

    try:
        new_amount = float(new_amount_str)
    except ValueError:
        print("❌ Amount must be numeric. Edit cancelled.")
        return entries
    ...
    entry["Description"] = sanitize_description(new_desc)
...
def main():
    ...
    while True:
        display_entries(entries)
        print("\nOptions: [A]dd entry  [E]dit entry  [R]emove entry  [S]ave  [P]ush to GitHub  [Q]uit")
